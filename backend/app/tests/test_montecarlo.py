"""Monte Carlo: percentile engine, sigma bands, and API endpoint + cache."""
...
# ---------- response cache (routers/projections) ----------


def _seed_cache_plan(owner_id: int) -> int:
...
def test_mc_cache_hits_within_ttl():
    ...
    with _as_user("mc-cache-user", "mc@example.com") as user:
        plan_id = _seed_cache_plan(user.id)
        with TestClient(app) as client:
