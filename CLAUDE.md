...
- **`engine/`** — pure functions, no SQLAlchemy. The simulator takes domain dataclasses (`PersonInput`, `IncomeInput`, etc. defined in `simulator.py`), not ORM rows. This is the layer to touch for any tax/pension/projection logic change.
  ...
  - `bik_ie.py` — benefit-in-kind cash equivalents (medical insurance, company car/van, preferential loan, `other`) plus the capped 20% medical-insurance relief credit. Pure primitives in / euros out, parameterised by `TaxConfig` (`bik_*` / `medical_insurance_relief_*` fields). The simulator charges the cash equivalent to IT/USC/PRSI as *notional pay* — see the BIK convention below.
...
- `claims_rent_credit` and `filing_status` — cohabiting couples are taxed individually under Irish law, so don't infer married filing from "2 people in plan".
- **Benefits-in-kind are notional pay, not cash.** A `Benefit`'s cash equivalent (via `engine/bik_ie.py`) is added to `taxable_for_it`/`taxable_for_usc`/`taxable_for_prsi` so it raises tax, but it is stripped back out of `true_gross` and never enters or leaves household cash (the employer funds it). It stops at retirement (employment-related), medical-insurance BIK also adds the capped relief credit via `FilingStatus.medical_insurance_relief`, and `YearRow.benefits_in_kind_total` reports it separately so it doesn't double-count in cash gross.
